...
static PyObject* httpheaders_PyTypeObject_iternextfunc(httpheaders_object *self)
{
        ...
        if (((keys_object*)key)->index == self->index)
	{
	    ...
	    PyObject *type_str = PyString_FromString(((keys_object*)key)->type_str);
	    if (type_str == NULL)
	    {
	        return (PyObject*)NULL;
	    }
...
PyObject* httpheaders_PyTypeObject_reprfunc(httpheaders_object *self)
{
        ...
        while (PyDict_Next(self->dict, &pos, &key, &value))
	{
	    ...
	    if (type_str != NULL)
	    {
	        if (strcmp(PyString_AsString(type_str), CONTENT_LENGTH) == 0)
		{
		    Py_ssize_t content_length = atoi(PyString_AsString(value));
   ...
   while(end_ptr - self->header < self->size) {
	 ...
	 if (colon_ptr) {
                  ...
                  if (key != NULL)
		  {
			  ...
			  *(key->type_str + len) = '\0';
			  value = PyString_FromStringAndSize(colon_ptr + 2, (end_ptr - 1) - (colon_ptr + 2));
			  if (value != NULL)
			  {
			      Py_XINCREF(key);
			      Py_XINCREF(value);
		...
		*/		  
		  ...
		  if (key != NULL)
		  {
			  ...
			  *(key->type_str + len) = '\0';
			  value = PyString_FromStringAndSize(begin_ptr, colon_ptr - begin_ptr);
			  if (value != NULL)
			  {
			      Py_XINCREF(key);
			      Py_XINCREF(value);
		  ...
		  if (foo && foo < end_ptr) 
		  {
			  ...
			  if (key->type_str != NULL)
			  {
			      ...
			      *(key->type_str + len) = '\0';
			      value = PyString_FromStringAndSize(colon_ptr, foo - colon_ptr);
			      if (value != NULL)
			      {
			          Py_XINCREF(key);
				  Py_XINCREF(value);
...
PyMODINIT_FUNC inithttpheaders(void) 
{
   ...
   module_httpheaders = Py_InitModule3("httpheaders", httpheadersMethods, HTTPHEADERS_VERSION_DOC);
